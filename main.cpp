...
void PrintAllHand(string                 wallname, 
									...
									string								 player3meldhandname,
									vector<vector<string>> player3meldhand,
									string 								 player3handname,
									vector<string> 				 player3hand,
									string      					 player4meldhandname,
									vector<vector<string>> player4meldhand,
									string 								 player4handname,
	...
	PrintMeldHand(player3meldhandname, player3meldhand, tile);
	PrintHand(player3handname, player3hand, tile, false);
	PrintMeldHand(player4meldhandname, player4meldhand, tile);
	PrintHand(player4handname, player4hand, tile, false);
}
...
void TileType :: SetValue(vector<string> calculatehand, 
													int 					 gameturn, 
													int 					 gameround, 
													bool 					 lastwalltile, 
													bool 					 tsumo, 
...
int main() {
	...
	bool player3tenpai = false;
	bool player4tenpai = false;
	// declare int
	int gameturn = 0; // #0 = player1, #1 = player2, #2 = player3, #3 = player4
	int gameround = 1;
	int meldselect;
	// declare string
	string player1handname = "玩家1手牌";
	string player2handname = "玩家2手牌";
	string player3handname = "玩家3手牌";
	string player4handname = "玩家4手牌";
	string player1meldhandname = "玩家1組合的手牌";
	string player2meldhandname = "玩家2組合的手牌";
	string player3meldhandname = "玩家3組合的手牌";
	string player4meldhandname = "玩家4組合的手牌";
	string wallname = "牌山";
	string rivername = "牌河";
	// declare map
	...
	vector<string> player3probablywintile;
	vector<string> player4probablywintile;
	...
	for (int i = 0; i < 2; i++) {
		...
		for (int j = 0; j < 2; j++)
			DrawCard(wallhand, player3hand);
		for (int j = 0; j < 2; j++)
			DrawCard(wallhand, player4hand);
	}
	...
	Sort(tile, player3hand);
	Sort(tile, player4hand);
	// print round
	...
	PrintAllHand( wallname, wallhand, 
		...
		player3meldhandname, player3meldhand,
		player3handname, player3hand,
		player4meldhandname, player4meldhand,
		player4handname, player4hand,
		tile);
	// check player1 is wining(tsumo-tenhou) or not
	gameswitch = CheckToWin(player1hand, tile, player1meldhand, riverhand);
	// if player1 isn't winning then go in to loop
	while (gameswitch) { // win by tsu-mo
		...
		if (gameturn == 0) {
			if (player4tenpai)
				AutoDisCard(player4hand,riverhand);
			else{
				PrintHand(player4handname, player4hand, tile, true);
				player4probablywintile = TenPaiCheck(player4hand,tile,player4meldhand);
				DeclareTenPai(player4probablywintile,player4tenpai);
				DisCard(player4hand, riverhand);
			}
			// sort player4hand after discard
			Sort(tile, player4hand);
			// print all hand
			PrintAllHand(wallname, wallhand, 
				player1handname, player1hand,
			...
			if (gameswitch) { // if player1isn't winning then check player3 is winning or tsumo
				// check player2 winning(ron)
				gameswitch = CheckToWin(player2hand, tile ,player2meldhand, riverhand);
				if (gameswitch){
					// check player3 winning(ron)
					gameswitch = CheckToWin(player3hand, tile ,player3meldhand, riverhand);
					if (gameswitch){
						if(!player1tenpai){
			...
			if (player1tenpai) // player1 is tenpai
				AutoDisCard(player1hand, riverhand); 
			else{ // player1 isn't tenpai
			...
			// sort player1hand after discard
			Sort(tile, player1hand);
			// print all hand
			PrintAllHand(wallname, wallhand, player1handname, player1hand,
				player1meldhandname, player1meldhand,	rivername, riverhand,
				player2meldhandname, player2meldhand,	player2handname, player2hand,
				player3meldhandname, player3meldhand,	player3handname, player3hand,
				player4meldhandname, player4meldhand, player4handname, player4hand,
				tile);
			// check player2 is winning(ron)
			gameswitch = CheckToWin(player2hand, tile, player2meldhand, riverhand); 
			// check player2 or player3 is winning(tsumo) or not
			if (gameswitch){	// if player2 isn't winning then check player3 is winning or tsumo
				// check player3 winning(ron)
				gameswitch = CheckToWin(player3hand, tile, player3meldhand,	riverhand); 
				if (gameswitch){ // if player3 isn't winning
					gameswitch = CheckToWin(player4hand, tile, player4meldhand,	riverhand); 
					if (gameswitch){ // if player3 isn't winning
						if(!player2tenpai){ //if player2 isn't tenpai 
							...
							if (meldselect != 0)  // if player2 have meld
								...
								DrawCard(wallhand, player2hand);
								// check player2 is winning or not 
								gameswitch = CheckToWin(player2hand, tile, player2meldhand, riverhand); 
								tsumo = gameswitch? false:true;
							}
						}else{ // else player2 is tenpai
							DrawCard(wallhand, player2hand);
							// check player2 is winning or not 
							gameswitch = CheckToWin(player2hand, tile, player2meldhand, riverhand); 
							tsumo = gameswitch? false:true;
			...
			if (player2tenpai) // player2 is tenpai
				AutoDisCard(player2hand, riverhand); 
			else{ // player2 isn't tenpai
			...
			// sort player2hand after discard
			Sort(tile, player2hand);
			// print all hand
			PrintAllHand(wallname, wallhand, player1handname, player1hand,
					player1meldhandname, player1meldhand,	rivername, riverhand,
					...
					player4meldhandname, player4meldhand,	player4handname, player4hand,
					tile);
			// check player1 is winning(ron)
			gameswitch = CheckToWin(player1hand, tile, player1meldhand, riverhand); 
			// check player1 or player3 is winning(tsumo) or not
			if (gameswitch){	// if player1 isn't winning then check player3 is winning or tsumo
				// check player3 winning(ron)
				gameswitch = CheckToWin(player3hand, tile, player3meldhand,	riverhand); 
				if (gameswitch){ // if player3 isn't winning
					gameswitch = CheckToWin(player4hand, tile, player4meldhand,	riverhand); 
					if (gameswitch){ // if player3 isn't winning
						if(!player3tenpai){ //if player3 isn't tenpai 
							checktomeld = CheckToMeld(player3hand, riverhand);
							meldselect = MeldSelect(checktomeld, tile);
							if (meldselect != 0)  // if player3 have meld
								MakeMeldFromPlayerHandToMeldHand(player3hand, player3meldhand,
										riverhand, checktomeld,	meldselect);
							else {
								DrawCard(wallhand, player3hand);
								gameswitch = CheckToWin(player3hand, tile, player3meldhand,riverhand);
			...
			if (player3tenpai) // player3 is tenpai
				AutoDisCard(player3hand, riverhand); 
			else{ // player3 isn't tenpai
				PrintHand(player3handname, player3hand, tile, true);
				player3probablywintile = TenPaiCheck(player3hand,tile,player3meldhand); 
				DeclareTenPai(player3probablywintile,player3tenpai);	
				DisCard(player3hand, riverhand);
			}
			// sort player3hand after discard
			Sort(tile, player3hand);
			// print all hand
			PrintAllHand(wallname, wallhand, player1handname, player1hand,
				player1meldhandname, player1meldhand,	rivername, riverhand,
			...
			if (gameswitch){	// if player1 isn't winning then check player3 is winning or tsumo
				// check player2 winning(ron)
				gameswitch = CheckToWin(player2hand, tile, player2meldhand,	riverhand); 
				if (gameswitch){ // if player2 isn't winning
					gameswitch = CheckToWin(player4hand, tile, player4meldhand,	riverhand); 
					if (gameswitch){ // if player4 isn't winning
						if(!player4tenpai){ //if player4 isn't tenpai 
							checktomeld = CheckToMeld(player4hand, riverhand);
							meldselect = MeldSelect(checktomeld, tile);
							if (meldselect != 0)  // if player3 have meld
								MakeMeldFromPlayerHandToMeldHand(player4hand, player4meldhand,
										riverhand, checktomeld,	meldselect);
							else {
								DrawCard(wallhand, player4hand);
								gameswitch = CheckToWin(player4hand, tile, player4meldhand,riverhand);
	...
	if (gameturn == 0) {
		...
		MakeCalculateScoreHand(calculatehand, player3hand, player3meldhand,
				riverhand.back());
		cout <<  "玩家3獲勝" <<  endl;
		tt.SetValue(calculatehand, gameturn, gameround, lastwalltile, tsumo, player3tenpai);
	}else if (gameturn == 3){
		MakeCalculateScoreHand(calculatehand, player4hand, player4meldhand,
				riverhand.back());
		cout <<  "玩家4獲勝" <<  endl;
		tt.SetValue(calculatehand, gameturn, gameround, lastwalltile, tsumo, player4tenpai);
	}
